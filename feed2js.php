...
$br = ' ';
if ($html == 'y') {
	// remove the requirement that if html is displayed, all feed content is displayed
	// $desc = 1;
} elseif ($html == 'p') {
	$br = '<br />';
}
...
if (isset($restrict_url) && substr($src_host, strlen($src_host)-strlen($restrict_url)) != $restrict_url) {
	...
	if  (!$rss) {
		...
		if ($chan == 'y') {
			...
			// output channel title, description and image	
			$str.= "document.write('<p class=\"rss-title-box\"><a class=\"rss-title-image-a\" href=\"" . trim($rss->image['link']) . "\"><img class=\"rss-title-image\" src=\"" . trim($rss->image['url']) . "\" alt=\"" . trim($rss->image['description']) . "\" \></a> <a class=\"rss-title\" href=\"" . trim($rss->channel['link']) . '"' . $target_window . ">" . addslashes(strip_returns($rss->channel['title'])) . "</a><br /><span class=\"rss-title-desc\">" . addslashes(strip_returns(strip_tags($rss->channel['description']))) . "</span></p>');\n";
		
		} elseif ($chan == 'title') {
			// output title only
			$str.= "document.write('<p class=\"rss-title-box\"><a class=\"rss-title\" href=\"" . trim($rss->channel['link']) . '"' . $target_window . ">" . addslashes(strip_returns($rss->channel['title'])) . "</a></p>');\n";
		
		} elseif ($chan == 'noimage') {
			// output title only
			$str.= "document.write('<p class=\"rss-title-box\"><a class=\"rss-title\" href=\"" . trim($rss->channel['link']) . '"' . $target_window . ">" . addslashes(strip_returns($rss->channel['title'])) . "</a><br /><span class=\"rss-title-desc\">" . addslashes(strip_returns(strip_tags($rss->channel['description']))) . "</span></p>');\n";
		
		}	elseif ($chan = 'n') {
			//do not display any channel information
		}
		
		// begin item listing
		$str.= "document.write('<div class=\"rss-items\">');\n";
		...
		foreach ( $all_items as $item ) {
			...
			if ($auth == 'y') {
				if (isset($item['dc']['creator'])) {
					$author_str = ' <span class="rss-item-author">(' . addslashes(strip_tags($item['dc']['creator'])) . ')</span>';
				} else if (isset($item['author'])) {		
					$author_str = ' <span class="rss-item-author">(' . addslashes(strip_tags($item['author'])) . ')</span>';
				} else if (isset($item['author_name'])) {
					$author_str = ' <span class="rss-item-author">(' . addslashes(strip_tags($item['author_name'])) . ')</span>';	
					}
			...
			if ($desc < 0) {
				$str.= "document.write('<div class=\"rss-item\">');\n";
				...
				$str.= "document.write('<div class=\"rss-item\"><a class=\"rss-item-title\" href=\"" . trim($my_url) . "\"" . $target_window . '>' . $my_title . '</a>' .  $author_str . "<br />');\n";
			...
			if ($play_podcast == 'y' and is_array($item['enclosure'])) {
				...
				for ($i = 0; $i < count($item['enclosure']); $i++) {
					...
					//if (strpos($item['enclosure'][$i]['url'], 'http://')!=0) {
						$str.= "document.write('<a class=\"pod-play\" href=\"" . trim($item['enclosure'][$i]['url']) . "\" title=\"Play Now\"><em>Play</em> <span> " .  substr(trim($item['enclosure'][$i]['url']), -3)  . "</span></a> ');\n";
					//}
			...
			$str.= "document.write('$more_link</div>');\n";	
		}
	
	
		$str .= "document.write('</div></div>');\n";
	} // end restrict_url
}
